...
**السؤال الذي يوجّه كل اختبار جديد:** _"لو تعطّل هذا السلوك تحديدًا، هل سيلاحظ المستخدم أو يخسر العمل شيئًا حقيقيًا؟"_ — إن كانت الإجابة نعم، يستحق اختبارًا. إن كانت الإجابة "فقط تفصيل داخلي لا يؤثر على المخرجات"، لا يستحق اختبارًا مخصصًا.
...
describe("calculateCartTotal", () => {
  ...
  it("يرمي خطأ عند تمرير كمية سالبة", () => {
    expect(() =>
      calculateCartTotal([{ unitPrice: 100, quantity: -1 }]),
...
_(الجزء 2 التالي: Accessibility Testing، API/Security/Performance Testing (مرجع + تكامل)، Test Data Management، Mocking Strategy، CI Test Orchestration، Coverage Targets، Flaky Test Management، Test Environments، Manual QA وBug Triage، Release Testing Checklist.)_
