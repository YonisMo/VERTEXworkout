...
export function apiError(
  ...
  return NextResponse.json(
    { success: false, error: { code, message, details }, meta: buildMeta() },
    { status },
  );
}
```
...
_(الجزء 3 التالي: الكتالوج الكامل لكل Endpoint حسب النطاق — Auth, Store, Programs, Exercise Library, Academy, Client, Notifications — مع أمثلة Request/Response كاملة لكل واحد.)_
